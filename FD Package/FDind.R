...
#  used in Vill?ger et al. 2009, Ecological Applications (in press)                                                      #
#                                                                                                                        #
# - Functional richness (FRic), Functional evenness (FEve), Functional divergence (FDiv)                                 #
#    Vill?ger et al. 2008 (Ecology, 89: 2290-2301)                                                                       #
#                                                                                                                        #
...
library(ape)
library(geometry)
traits <- as.matrix(coord)#as.matrix(coord)
abundances <- as.matrix(weight)#as.matrix(weight)
...
res<-data.frame(Nbsp=Nbsp, FRic=FRic,FEve=FEve, FDiv=FDiv, FSpe=FSpe ) ; row.names(res)<-row.names(abundances)
invisible(res)
...
traitsex<-matrix(runif(15,0,10),5,3,dimnames=list(paste("sp",1:5,sep=""),c("tr1","tr2","tr3")))
abundex<-matrix(c(0,runif(26,0,100),NA,0,0),6,5,dimnames=list(paste("st",1:6,sep=""),paste("sp",1:5,sep="")))
ex<-FDind(traitsex,abundex)
print(traitsex) ; print (abundex) ; print(ex)
